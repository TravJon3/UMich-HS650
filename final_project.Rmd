...
1. Use feature selection models (Boruta and Stepwise Feature Selection) given the nature of my study.
2. Use clustering nearest neighbors (kNN, k-Means) using one-hot encoding because via clustering I can determine the trends within the clusters that may be predictive in nature. (and check with SVM.)  --- not sure
3. I chose not to use apriori techniques because it expects the itemsets to have repetition.
...
df_display <- convertDataframeDisplay(df_full_non_int)
...
df_objective_display <- convertDataframeDisplay(df_objective_non_int)
...
We will compare the mean and median for data that has been imputed vs data that have not been imputed.  While the patterns appear similar, the y-axis for both the means and medians are vastly different.  To preserve the integrity of the data, I will not utilize imputation.
...
set.seed(1234)
#df_full_impute_hmisc <- as.data.frame(Hmisc::aregImpute(as.matrix(PositiveChange ~ ., data = df_full_na, x=TRUE)))
df_full_impute_forest <- as.data.frame(missForest::missForest(as.matrix(df_full_na), maxiter=2)$ximp)
# Medians
#plot_summary(df_full_impute_hmisc, "Full Dataset - Multiple Imputation")[[1]]
plot_summary(df_full_impute_forest, "Full Dataset - Random Forest")[[1]]
plot_summary(df_full_na, "Full Dataset - Non Imputed")[[1]]
# Means
#plot_summary(df_full_impute_hmisc, "Full Dataset - Multiple Imputation")[[2]]
plot_summary(df_full_impute_forest, "Full Dataset - Random Forest")[[2]]
plot_summary(df_full_na, "Full Dataset - Non Imputed")[[2]]
```
# Summary
