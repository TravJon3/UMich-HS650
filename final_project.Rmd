...
plot_violin <- function(df, string_type) {
  sum_mat <- summary(df)
  vec_medians <- sum_mat[3, ]
  vec_means <- sum_mat[4, ]
  vec_fieldnames <- trimws(colnames(sum_mat))
  ...
  for (i in 1:ncol(df)) {
    p <- p %>% add_trace(y = ~df[, colnames(df)[i]], name=colnames(df)[i], box = list(visible = TRUE), meanline = list(visible = TRUE))
  }
  p <- p %>% layout(title=paste0("Distribution of Means of Summary for ", string_type), yaxis = list(title = "Count", xaxis = list(title = "Field Names")))
  p
}
...
# test <- lapply(df_full_na, as.numeric)
# vioplot(PositiveChange ~ ., data=df_full_na, Centre = "line", side = "left")
# #vioplot(Sepal.Length~Species, data=iris_small, col = "lightblue", plotCentre = "line", side = "right", add = T)
