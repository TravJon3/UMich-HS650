...
<p>Not going to do RFE because Boruta is based on RFE.  Stepwise feature selection and PCA confirmation.  Multiple Factor Analysis</p>
...
set.seed(1234)
boruta_full <- Boruta(PositiveChange ~ ., data = df_full)
print(boruta_full)
...
set.seed(1234)
boruta_objective <- Boruta(PositiveChange ~ ., data = df_objective)
...
ols_objective <- run_stepwise(df_objective)
...
set.seed(1234)
# pca_full <- prcomp(as.matrix(df_full_onehot))
# eigen_full <- get_eigenvalue(pca_full)
# eigen_full
# plot_ly(x = c(1:length(pca_full$sdev)), y = pca_full$sdev*pca_full$sdev, name = "Scree", type = "bar") %>%
#   layout(title="Scree Plot", xaxis = list(title="PC's"),  yaxis = list(title="Variances (SD^2)"))

# ICA_full <- fastICA(df_full_onehot, k_full, alg.typ = "parallel", fun = "logcosh", alpha = 1, 
